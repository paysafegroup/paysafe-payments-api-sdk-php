...
interface CustomerPaymentHandleServiceInterface
{
    /**
     ...
     *
     * <p><b>Note:</b> Currently, only CARD, ACH, EFT, BACS and SEPA
     * are supported as a paymentType parameter in Payments API.</p>
     * <p><b>Note:</b> For card processing, the paymentHandleToken returned
     * in the response can be used for payment requests in the Payments API,
     * once it has the status of PAYABLE. To modify its status to PAYABLE,
     * include this paymentHandleToken as the value for the
     * paymentHandleTokenFrom parameter in a request to create a regular Payment Handle. <br />
     * That response in turn contains a paymentHandleToken with a status of PAYABLE,
     * with the usage parameter set to SINGLE_USE.
     * Once you use this single-use paymentHandleToken in a successful payment request,
     * then the status of the Customer paymentHandleToken
     * created here will change to PAYABLE.</p>
     * <p><b>Note: </b> Although the billingDetails parameter is optional,
     * it is recommended that you include it when you create the customer
     * Payment Handle because a Payment request that uses the paymentHandleToken in the response will fail AVS checks
     * unless there is a billing address
     * associated with it. If you do not include it when creating the customer Payment Handle,
     * you should then update the Payment Handle with the
     ...
     *
     * <p><b>Note: </b> Currently, single-use Payment Handle tokens that have CARD, ACH and EFT
     * as the paymentType are supported.</p>
     *
     * <p>Endpoint:
     * <strong>POST /v1/customers/{customerId}/paymenthandles</strong></p>
     *
     * @param string $customerId                   id of a customer for whom to create a payment handle
     * @param CustomerPaymentHandleRequest $customerPaymentHandleRequest containing data needed to create
     *                                                                      a payment handle for customer
    ...
    */
     ...
     * Uses custom request options (connectTimeout, responseTimeout, automaticRetries and/or simulator, if applicable),
     * instead of values from PaysafeClient.</p>
     *
     ...
     *
     * <p><b>Use Case:</b> You can use this method to check the status of a Payment Handle
     * (e.g., PAYABLE, EXPIRED, etc.)
     ...
     * <p>
     * @param string $customerId      The unique identifier of the customer for whom the payment handle was created.
     *                        This ID is returned in the response when a customer is created.
     * @param string $paymentHandleId The unique identifier of the payment handle created for the customer.
     *                        This ID is returned in the response
     *                          to the "Create Payment Handle for a Customer" request.
     * @param RequestOptions|null $requestOptions  Custom connectTimeout, responseTimeout, automaticRetries
     *                          and/or simulator (if applicable) for this request.
     *
     * @return CustomerPaymentHandle if the retrieval is successful
     *
    ...
    */
    ...
    /**
     * <p>Update a Customer Payment Handle using its ID and customer ID.
     * The request will be executed using custom RequestOptions
     * (connectTimeout, readTimeout, maxAutomaticRetries and/or simulator,
     * if applicable), instead of default values from PaysafeClient.</p>
     *
     ...
     * @param string $paymentHandleId              the ID of the payment handle to update
     * @param CustomerPaymentHandleRequest $customerPaymentHandleRequest updated data for the payment handle
     * @param RequestOptions|null $requestOptions custom options for the request
     *
    ...
    */
     ...
     * automaticRetries and/or simulator, if applicable), instead of values from PaysafeClient.
     *
     * <p>Endpoint:
     ...
     * @param string $paymentHandleId the unique ID returned in the response to the Create a Payment Handle
